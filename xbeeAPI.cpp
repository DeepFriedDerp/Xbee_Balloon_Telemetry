...
accModeLevel0_sense      PROTOTHREAD
***************************************************************************/
int Xbee::receiverMode_sense(struct pt *pt)
{
  #ifdef debug_receiverMode_sense
    Serial.println(F("receiverMode_sense : called"));
  #endif

 PT_BEGIN(pt);
   PT_WAIT_UNTIL(pt, (!commandMode && (_MODE==2) && (_packetParsed)));
   #ifdef debug_receiverMode_sense
     Serial.println(F("receiverMode_sense : triggered"));
   #endif
   Xbee::resetStatus();
 PT_END(pt);

 #ifdef debug_receiverMode_sense
   Serial.println(F("receiverMode_sense : exit"));
 #endif
}
    ...
    switch (_MODE)
    {
      ...
      case 2:
        ...
        if(frametype == 0x90)
        {
          ...
          data_received_length = _data_received_length-12;
          for(int i = 12; i < _data_received_length; i++)
          {
          ...
          functionState = true;
            ...
            for(int i = 0; i < data_received_length; i++)
            {
